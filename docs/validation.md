...
recovery, local share checks, and Signet address previews. Thirty frontend
...
Error correction is implemented as `Codex32::correct` (unique closest valid string, returned for user confirmation only) plus the exported bech32 `CorrectableError`/`Corrector` path for erasures with known locations; both are covered by tests across every published string, position, and substitute character in both checksum formats. Native mobile bindings, protected key storage, production chain adapters, atomic persistence, a full wallet interface, broader device/browser testing, sustained fuzzing, and independent security review remain. The wallet disables mainnet; the backup core is a general BIP 93 implementation and should still be treated as unaudited. See the [mainnet readiness record](mainnet-readiness.md) for the remaining evidence and integration requirements.
