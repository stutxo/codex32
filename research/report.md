...
| Executable creation state machine | Fixed profile binds protocol version, nonce, account, attested endpoint, expiry, roles, indices, and both commitments; wrong order, tamper, changed context, malformed metadata, expiry, duplicate-session reservation, and in-object replay fail closed | Use its digest as reviewed HPKE/AEAD associated data; callers must implement the required durable atomic replay store |
...
- [`crates/codex32-wallet/src/ceremony.rs`](../crates/codex32-wallet/src/ceremony.rs): fixed-profile transcript state machine, canonical delivery associated data, and fail-closed atomic replay-store boundary. Transport encryption and durable storage are deliberately delegated to reviewed implementations.
...
The transcript state machine makes commit-before-reveal ordering and context binding executable. It rejects altered nonce/account/endpoint/expiry, mismatched role/index/identifier/threshold/length, wrong associated data, expired sessions, wrong ordering, and replay within a live ceremony object. Before beginning, it requires a caller-supplied store to atomically reserve a domain-separated identifier for the complete context; duplicate reservation or store failure returns `CeremonyError::Replay`.

It intentionally does not implement encryption or storage. Production must supply reviewed HPKE/AEAD and a durable, atomic, fail-closed replay store. The prototype does not prove entropy quality, constant-time behavior, secure deletion, hardware attestation, transport-cryptography security, replay-store durability, physical security, company operations, TEE behavior, or production fitness. Deterministic generators and every emitted secret are fixed public test data. Never fund them.
...
4. Can a second implementation reproduce the canonical identity definition—SHA-256 tag bytes `codex32/wallet-identity/v1\0`, raw 32-byte network genesis `ChainHash`, big-endian `u32` byte length plus canonical external public descriptor, then the same encoding for the canonical internal public descriptor—and the fixed regtest vector `adb35db5873ab9d3ba0c4b4b0a8e78b276b4a85fb48e983e6ba4a624771eb0bd` for the 32-byte seed `05` repeated 32 times?
...
7. Which reviewed HPKE/AEAD profile, hardware attestation format, key-confirmation step, and durable atomic replay-store implementation should carry the state machine's transcript-bound company share? The implemented state transitions and store interface do not prove channel cryptography, persistence, or fairness.
